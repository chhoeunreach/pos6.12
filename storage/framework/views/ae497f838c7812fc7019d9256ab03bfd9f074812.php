...
<?php
    ...
    $view_suspended_sell_url = route('service.sells.index') . '?suspended=1';
    $pos_redirect_url = '/' . trim(config('service.route_prefix', 'service-pos'), '/') . '/pos/create';
...
<div class="col-md-12 no-print pos-header">
    ...
    <div
        ...
        <div class="tw-w-full md:tw-w-1/3">
            <div class="tw-flex tw-items-center tw-gap-2">
                ...
                <div
                    ...
                    <span class="curr_datetime text-white tw-font-semibold tw-text-xs tw-leading-none"><?php echo e(\Carbon::createFromTimestamp(strtotime('now'))->format(session('business.date_format') . ' ' . 'H:i'), false); ?></span>
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-keyboard hover-q text-white tw-opacity-70 tw-cursor-pointer" width="14" height="14" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" data-container="body" data-toggle="popover" data-placement="bottom" data-content="<?php echo $__env->make('service::sale_pos.partials.keyboard_shortcuts_details', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>" data-html="true" data-trigger="hover" data-original-title="" title=""><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M2 6m0 2a2 2 0 0 1 2 -2h16a2 2 0 0 1 2 2v8a2 2 0 0 1 -2 2h-16a2 2 0 0 1 -2 -2z"/><path d="M6 10l0 .01"/><path d="M10 10l0 .01"/><path d="M14 10l0 .01"/><path d="M18 10l0 .01"/><path d="M6 14l0 .01"/><path d="M18 14l0 .01"/><path d="M10 14l4 0"/></svg>
                </div>
                ...
                <span class="tw-block md:tw-hidden <?php if(empty($pos_settings['hide_product_suggestion'])): ?> tw-ml-0 <?php else: ?> tw-ml-auto <?php endif; ?>">
        ...
        <div class="tw-w-full md:tw-w-2/3 !tw-p-0 tw-flex tw-items-center md:tw-justify-end tw-gap-2 md:tw-gap-1 tw-flex-col md:tw-flex-row tw-hidden md:tw-flex"
            ...
            <a href="<?php echo e($main_products_url, false); ?>" title="Back to <?php echo e($main_project_name, false); ?>" data-service-skip-rewrite="true"
                ...
                <strong class="tw-inline-flex tw-items-center tw-gap-1.5">
                    ...
                    Back to <?php echo e($main_project_name, false); ?>
            ...
            <?php if(auth()->user()->can('view_cash_register') || auth()->user()->can('close_cash_register')): ?>
                <span class="pos-nav-divider tw-inline-block tw-w-px tw-h-[18px] tw-bg-[#e2e8f0] tw-flex-shrink-0 tw-self-center tw-rounded-[1px] tw-mx-[3px]"></span>
            <?php endif; ?>
            ...
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('view_cash_register')): ?>
                <button type="button" id="register_details" title="<?php echo e(__('cash_register.register_details'), false); ?>"
                    ...
                    data-href="<?php echo e(url(config('service.route_prefix', 'service-pos') . '/cash-register/register-details'), false); ?>">
            ...
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('close_cash_register')): ?>
                <button type="button" id="close_register" title="<?php echo e(__('cash_register.close_register'), false); ?>"
                    ...
                    data-href="<?php echo e(url(config('service.route_prefix', 'service-pos') . '/cash-register/close-register'), false); ?>">
                    <strong class="!tw-m-3">
            ...
            <?php if(!empty($pos_settings['inline_service_staff']) || in_array('tables', $enabled_modules) || in_array('service_staff', $enabled_modules)): ?>
                <span class="pos-nav-divider tw-inline-block tw-w-px tw-h-[18px] tw-bg-[#e2e8f0] tw-flex-shrink-0 tw-self-center tw-rounded-[1px] tw-mx-[3px]"></span>
            <?php endif; ?>
            ...
            <?php if(!empty($pos_settings['inline_service_staff'])): ?>
                <button type="button" id="show_service_staff_availability"
                    ...
                    data-href="<?php echo e(url(config('service.route_prefix', 'service-pos') . '/service-staff-availability'), false); ?>">
                    <strong class="!tw-m-3">
            ...
            <button title="<?php echo app('translator')->get('lang_v1.calculator'); ?>" id="btnCalculator" type="button"
                class="tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border-2 tw-flex tw-items-center tw-justify-center tw-rounded-md md:tw-w-10 tw-w-auto tw-h-10 tw-text-gray-600 active:tw-scale-95 tw-transition-transform pull-right popover-default"
                data-toggle="popover" data-trigger="click" data-content='<?php echo $__env->make('service::layouts.partials.calculator', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>' data-html="true"
            ...
            <?php if(!empty($pos_settings['customer_display_screen'])): ?>
                <a href="<?php echo e(route('service.pos_display'), false); ?>" id="customer_display_screen"  onclick="window.open(this.href, 'customer_display', 'width='+screen.width+',height='+screen.height+',top=0,left=0'); return false;"   title="<?php echo e(__('lang_v1.customer_display_screen'), false); ?>"
                    class="tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border-2 tw-flex tw-items-center tw-justify-center tw-rounded-md md:tw-w-10 tw-w-auto tw-h-10 tw-text-gray-600 active:tw-scale-95 tw-transition-transform pull-right">
                    <strong class="!tw-m-3">
            ...
            <?php if((in_array('pos_sale', $enabled_modules) && !empty($transaction_sub_type) && auth()->user()->can('sell.create')) || auth()->user()->can('expense.add')): ?>
                <span class="pos-nav-divider tw-inline-block tw-w-px tw-h-[18px] tw-bg-[#e2e8f0] tw-flex-shrink-0 tw-self-center tw-rounded-[1px] tw-mx-[3px]"></span>
            <?php endif; ?>
            ...
            <?php if(in_array('pos_sale', $enabled_modules) && !empty($transaction_sub_type)): ?>
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sell.create')): ?>
                    <a href="/<?php echo e(trim(config('service.route_prefix', 'service-pos'), '/'), false); ?>/pos/create"
                        ...
                        <strong class="tw-inline-flex tw-items-center tw-gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-layout-grid tw-text-[#00935F]" width="18" height="18" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M4 4m0 1a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v4a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1z"/><path d="M14 4m0 1a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v4a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1z"/><path d="M4 14m0 1a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v4a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1z"/><path d="M14 14m0 1a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v4a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1z"/></svg><?php echo app('translator')->get('sale.pos_sale'); ?></strong>
                    </a>
                <?php endif; ?>
            <?php endif; ?>
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('expense.add')): ?>
                <button type="button" title="<?php echo e(__('expense.add_expense'), false); ?>" data-placement="bottom"
                    ...
                    id="add_expense">
                    <strong class="tw-inline-flex tw-items-center tw-gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-circle-minus" width="18" height="18" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 12a9 9 0 1 0 18 0a9 9 0 0 0 -18 0"/><path d="M9 12l6 0"/></svg><?php echo app('translator')->get('expense.add_expense'); ?></strong>
                </button>
            <?php endif; ?>
...
<?php /**PATH C:\xampp\htdocs\apply like facebook\pos6.12\Modules/Service\Resources/views/layouts/partials/header-pos.blade.php ENDPATH**/ ?>
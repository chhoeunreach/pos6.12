...
<aside class="side-bar tw-relative tw-hidden tw-h-full tw-bg-white tw-w-64 xl:tw-w-64 lg:tw-flex lg:tw-flex-col tw-shrink-0">
    ...
    <a href="<?php echo e(route('accessory.home'), false); ?>"
    ...
    <?php
        ...
        $baseUrl = url('/');
        $accessoryPrefix = trim(config('accessory.route_prefix', 'accessory-pos'), '/');
        $accessoryUrl = url($accessoryPrefix);

        $menuHtml = preg_replace_callback('/href="([^"]*)"/', function ($matches) use ($baseUrl, $accessoryUrl) {
            $href = $matches[1];
            ...
            if (
                ...
                str_starts_with($href, $accessoryUrl)
            ) {
                return $matches[0];
            }
            ...
            if (str_starts_with($href, $baseUrl)) {
                ...
                if ($path !== '' && ! str_starts_with($path, $routePrefix)) {
                    return 'href="' . $accessoryUrl . '/' . $path . '"';
                }
            }
...
<?php /**PATH C:\xampp\htdocs\apply like facebook\pos6.12\Modules/Accessory\Resources/views/layouts/partials/sidebar.blade.php ENDPATH**/ ?>
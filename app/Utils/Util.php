...
class Util
{
    ...
    public function num_f($input_number, $add_symbol = false, $business_details = null, $is_quantity = false)
    {
        ...
        $thousand_separator = ! empty($business_details) ? $business_details->thousand_separator : ($currency_data['thousand_separator'] ?? '');
        $decimal_separator = ! empty($business_details) ? $business_details->decimal_separator : ($currency_data['decimal_separator'] ?? '');
        ...
        if ($add_symbol) {
            ...
            $symbol = ! empty($business_details) ? $business_details->currency_symbol : ($currency_data['symbol'] ?? '');
    ...
    {
        // Request-level cache to avoid N+1 queries on forms that call this repeatedly.
        static $subUnitCache = [];
        $cacheKey = implode(':', [
            (string) $business_id,
            (string) $unit_id,
            $return_main_unit_if_empty ? '1' : '0',
            (string) ($product_id ?? 0),
        ]);
        if (array_key_exists($cacheKey, $subUnitCache)) {
            return $subUnitCache[$cacheKey];
        }
        ...
        $subUnitCache[$cacheKey] = $sub_units;
    ...
    public function getContactDue($contact_id, $business_id = null)
    {
        ...
        $query = Contact::where('contacts.id', $contact_id)
            ...
            ->select(
                ...
                DB::raw("SUM(IF(t.status = 'final' AND t.type = 'sell', COALESCE(tp.net_amount, 0), 0)) as total_paid"),

<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up()
    {
        Schema::table('notification_templates', function (Blueprint $table) {
            if (!Schema::hasColumn('notification_templates', 'auto_send_sms')) {
                $table->boolean('auto_send_sms')->after('auto_send')->default(0);
            }
            if (!Schema::hasColumn('notification_templates', 'cc')) {
                $table->string('cc')->nullable()->after('subject');
            }
            if (!Schema::hasColumn('notification_templates', 'bcc')) {
                $table->string('bcc')->nullable()->after('cc');
            }
            if (!Schema::hasColumn('notification_templates', 'whatsapp_text')) {
                $table->text('whatsapp_text')->nullable()->after('sms_body');
            }
            if (!Schema::hasColumn('notification_templates', 'auto_send_wa_notif')) {
                $table->boolean('auto_send_wa_notif')->after('auto_send_sms')->default(0);
            }
        });
    }

    public function down()
    {
    }
};

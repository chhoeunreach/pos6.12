...
<section class="content no-print">
    ...
    <div class="hide print_table_part">
        ...
        <div style="width: 100%;">
            <div class="info_col">
                @include('service::contact.contact_basic_info')
            </div>
            <div class="info_col">
                @include('service::contact.contact_more_info')
            </div>
            @if( $contact->type != 'customer')
                <div class="info_col">
                    @include('service::contact.contact_tax_info')
                </div>
            @endif
            <div class="info_col">
                @include('service::contact.contact_payment_info')
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-solid">
                <div class="box-body">
                    @include('service::contact.partials.contact_info_tab')
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="nav-tabs-custom">
                ...
                <div class="tab-content">
                    <div class="tab-pane
                                ...
                                @endif"
                            id="ledger_tab">
                        @include('service::contact.partials.ledger_tab')
                    </div>
                    @if(in_array($contact->type, ['both', 'supplier']))
                        <div class="tab-pane
                            @if(!empty($view_type) &&  $view_type == 'purchase')
                        ...
                        id="purchases_tab">
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    @include('service::purchase.partials.purchase_table')
                                </div>
                            </div>
                        </div>
                        <div class="tab-pane 
                            ...
                            @include('service::contact.partials.stock_report_tab')
                        </div>
                    @endif
                    @if(in_array($contact->type, ['both', 'customer']))
                        <div class="tab-pane 
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    @component('service::components.widget')
                                        @include('service::sell.partials.sell_list_filters', ['only' => ['sell_list_filter_payment_status', 'sell_list_filter_date_range', 'only_subscriptions']])
                                    @endcomponent
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    @include('service::sale_pos.partials.sales_table')
                                </div>
                            </div>
                        </div>
                        @if(in_array('subscription', $enabled_modules))
                            @include('service::contact.partials.subscriptions')
                        @endif
                    @endif
                    <div class="tab-pane
                            @if(!empty($view_type) &&  $view_type == 'documents_and_notes')
                        ...
                        @include('service::contact.partials.documents_and_notes_tab')
                    </div>
                    <div class="tab-pane 
                        @if(!empty($view_type) &&  $view_type == 'payments')
                            active
                    ...
                    <div class="tab-pane"
                        id="activities_tab">
                        @include('service::activity_log.activities')
                    </div>
...
@include('service::ledger_discount.create')
...
$(document).ready( function(){
    ...
    supplier_stock_report_table = $('#supplier_stock_report_table').DataTable({
        ...
        fixedHeader:false,
        'ajax': {
            url: "{{action([\Modules\Service\Http\Controllers\ContactController::class, 'getSupplierStockReport'], [$contact->id])}}",
            data: function (d) {
    ...
    $('#contact_id').change( function() {
        if ($(this).val()) {
            var contactUrl = "{{ route('service.contacts.show', ['contact' => '__contact_id__']) }}";
...
function get_contact_payments(url = null) {
    if (!url) {
        url = "{{action([\Modules\Service\Http\Controllers\ContactController::class, 'getContactPayments'], [$contact->id])}}";
    }
    $.ajax({
        url: url,
        dataType: 'html',
...
$(document).on('click', '#send_ledger', function() {
    ...
    var url = "{{action([\Modules\Service\Http\Controllers\NotificationController::class, 'getTemplate'], [$contact->id, 'send_ledger'])}}" + '?start_date=' + start_date + '&end_date=' + end_date + '&format=' + format + '&location_id=' + location_id;
...
@include('service::sale_pos.partials.sale_table_javascript')
<script src="{{ asset('modules/service/v7/js/payment.js?v=' . $asset_v . '-service-jsfix-20260609') }}"></script>
@if(in_array($contact->type, ['both', 'supplier']))
    <script src="{{ asset('js/purchase.js?v=' . $asset_v . '-service-jsfix-20260609') }}"></script>
@endif

<!-- document & note.js -->
@include('service::documents_and_notes.document_and_note_js')
@if(!empty($contact_view_tabs))
    @foreach($contact_view_tabs as $key => $tabs)
        @foreach ($tabs as $index => $value)
...
@include('service::sale_pos.partials.subscriptions_table_javascript', ['contact_id' => $contact->id])
@endsection

...
@section('content')
                ...
                <div class="row tw-flex lg:tw-flex-row md:tw-flex-col sm:tw-flex-col tw-flex-col tw-items-stretch" style="gap: 4px; margin: 0; padding: 0;">
                        ...
                        <div class="tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-rounded-2xl tw-bg-white" style="padding:0;overflow:hidden;height:100%;">
                            ...
                            {{-- <div class="box box-solid mb-12 @if (!isMobile()) mb-40 @endif"> --}}
                                <div class="box-body pb-0">
                                    ...
                                    @include('service::sale_pos.partials.pos_form')

                                    @include('service::sale_pos.partials.pos_form_totals')
                    ...
                    @if (empty($pos_settings['hide_product_suggestion']))
                        <div class="tw-w-full lg:tw-w-[40%]" style="padding:0;" id="pos_sidebar_wrap">
                            @include('service::sale_pos.partials.pos_sidebar')
                        </div>
                    @endif
                </div>
            </div>
        </div>
        @include('service::sale_pos.partials.pos_form_actions')
        {!! Form::close() !!}
    </section>
    ...
    @if (empty($pos_settings['hide_product_suggestion']) && isMobile())
        @include('service::sale_pos.partials.mobile_product_suggestions')
    @endif
    <!-- /.content -->
    ...
    @include('service::sale_pos.partials.recent_transactions_modal')

    @include('service::sale_pos.partials.weighing_scale_modal')
...
@section('javascript')
    ...
    @php
        $service_pos_public = public_path('modules/service/v7/js/pos.js');
        $service_pos_mtime = file_exists($service_pos_public)
            ? filemtime($service_pos_public)
            : time();
    @endphp
    <script src="{{ asset('modules/service/v7/js/pos.js?v=' . $asset_v . '&m=' . $service_pos_mtime) }}"></script>
    ...
    @if (!empty($pos_module_data))
        ...
        @endforeach
    @endif
    @include('service::sale_pos.partials.pos_layout_script', ['form_id' => 'add_pos_sell_form'])
@endsection

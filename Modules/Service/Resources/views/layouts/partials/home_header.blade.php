  ...
  <div class="container">
    ...
    <div id="navbar" class="navbar-collapse collapse">
      <ul class="nav navbar-nav">
        @if(Auth::check())
            <li><a href="{{ action([\Modules\Service\Http\Controllers\HomeController::class, 'index']) }}">@lang('home.home')</a></li>
        @endif
        ...
        @endif
        @if(!in_array('Repair', config('service.excluded_main_modules', []), true) && Route::has('repair-status'))
        <li>
      ...
      <ul class="nav navbar-nav navbar-right">
        @if (Route::has('login'))
            @if(!Auth::check())
                <li><a href="{{ route('service.login') }}">@lang('lang_v1.login')</a></li>
                @if(config('constants.allow_registration'))
                    <li><a href="{{ route('service.business.getRegister') }}">@lang('lang_v1.register')</a></li>
                @endif
            @endif
        @endif
      </ul>

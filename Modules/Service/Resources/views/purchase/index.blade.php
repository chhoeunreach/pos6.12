...
@section('content')
    ...
    <section class="content no-print">
        @component('service::components.filters', ['title' => __('report.filters')])
            <div class="col-md-3">
        ...
        @component('service::components.widget', ['class' => 'box-primary', 'title' => __('purchase.all_purchases')])
            @can('purchase.create')
                @slot('tool')
                    <div class="box-tools">
                        {{-- <a class="btn btn-block btn-primary" href="{{action([\Modules\Service\Http\Controllers\PurchaseController::class, 'create'])}}">
                    <i class="fa fa-plus"></i> @lang('messages.add')</a> --}}
                        <a class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right"
                            href="{{action([\Modules\Service\Http\Controllers\PurchaseController::class, 'create'])}}">
        ...
        @include('service::purchase.partials.update_purchase_status_modal')
...
@section('javascript')
    ...
    <script>
        ...
        var purchaseTableUrl = @json(action([\Modules\Service\Http\Controllers\PurchaseController::class, 'index']));
        var purchaseDefaultDateRangeDays = 30;
    </script>
    <script src="{{ asset('js/purchase.js?v=' . $asset_v . '-service-jsfix-20260609' . '&m=' . filemtime(public_path('js/purchase.js'))) }}"></script>
    <script src="{{ asset('modules/service/v7/js/payment.js?v=' . $asset_v . '-service-jsfix-20260609') }}"></script>
    <script>
        //Date range as a button
        var purchaseDefaultStart = moment().subtract(purchaseDefaultDateRangeDays - 1, 'days');
        var purchaseDefaultEnd = moment();
        var purchaseDateRangeSettings = $.extend({}, dateRangeSettings, {
            startDate: purchaseDefaultStart,
            endDate: purchaseDefaultEnd
        });

        $('#purchase_list_filter_date_range').val(
            purchaseDefaultStart.format(moment_date_format) + ' ~ ' + purchaseDefaultEnd.format(moment_date_format)
        );

        $('#purchase_list_filter_date_range').daterangepicker(
            purchaseDateRangeSettings,
            function(start, end) {
                $('#purchase_list_filter_date_range').val(start.format(moment_date_format) + ' ~ ' + end.format(
                    moment_date_format));
                reloadPurchaseTable(false);
            }
        );

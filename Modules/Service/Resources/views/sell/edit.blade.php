...
@section('css')
@include('service::sell.partials.direct_sell_table_styles')
@stop
...
<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
	{!! Form::open(['url' => action([\Modules\Service\Http\Controllers\SellPosController::class, 'update'], ['po' => $transaction->id ]), 'method' => 'put', 'id' => 'edit_sell_form', 'files' => true, 'data-transaction-id' => $transaction->id ]) !!}
	...
	<div class="row">
		<div class="col-md-12 col-sm-12">
			@component('service::components.widget', ['class' => 'box-solid'])
				...
				@if(in_array('types_of_service', $enabled_modules) && !empty($transaction->types_of_service))
					...
					<div class="modal fade types_of_service_modal" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">
						...
						@include('service::types_of_service.pos_form_modal', ['types_of_service' => $transaction->types_of_service])
						@endif
					</div>
				@endif
		        ...
		        <div class="col-sm-3">
	                <div class="form-group">
	                    ...
	                    @includeIf('service::components.document_help_text')</p>
	                </div>
	            </div>
		        <div class="clearfix"></div>
					...
					<table class="table table-condensed table-bordered table-striped table-responsive direct-sell-product-table" id="pos_table">
						...
						<tbody>
							@foreach($sell_details as $sell_line)
								@include('service::sale_pos.product_row', ['product' => $sell_line, 'row_count' => $loop->index, 'tax_dropdown' => $taxes, 'sub_units' => !empty($sell_line->unit_details) ? $sell_line->unit_details : [], 'action' => 'edit', 'is_direct_sell' => true, 'so_line' => $sell_line->so_line, 'is_sales_order' => $transaction->type == 'sales_order', 'is_serial_no' => true])
							@endforeach
						</tbody>
					</table>
					</div>
				...
				<div class="col-sm-10 col-sm-offset-1">
					<div class="form-group">
						<div class="input-group">
							...
							<span class="input-group-btn">
								<button type="button" class="btn btn-default bg-white btn-flat pos_add_quick_product" data-href="{{action([\Modules\Service\Http\Controllers\ProductController::class, 'quickAdd'])}}" data-container=".quick_add_product_modal"><i class="fa fa-plus-circle text-primary fa-lg"></i></button>
							</span>
						</div>
					</div>
				</div>
	        ...
	        <div class="col-md-4">
                <div class="form-group">
                    ...
                    <p class="help-block">
                    	...
                    	@includeIf('service::components.document_help_text')
                    </p>
                    @php
                    	$medias = $transaction->media->where('model_media_type', 'shipping_document')->all();
                    @endphp
                    @include('service::sell.partials.media_table', ['medias' => $medias, 'delete' => true])
                </div>
            </div>
	        <div class="clearfix"></div>
			...
			@if(!empty($common_settings['is_enabled_export']) && $transaction->type != 'sales_order')
				@component('service::components.widget', ['class' => 'box-solid', 'title' => __('lang_v1.export')])
	...
	@if($is_enabled_download_pdf && $transaction->type != 'sales_order')
		@can('sell.payments')
			@component('service::components.widget', ['class' => 'box-solid', 'title' => __('purchase.add_payment')])
				<div class="well row">
					<div class="col-md-6">
						<div class="form-group">
	...
	@can('sell.payments')
		...
		<div class="row">
			@foreach($payment_lines as $payment_line)			
				...
				<div class="payment_row" id="payment_rows_div">
					@include('service::sale_pos.partials.payment_row_form', ['row_index' => $loop->index, 'show_date' => true, 'payment_line' => $payment_line, 'show_denomination' => true])
				</div>
				@endforeach
		...
		<div class="row @if($change_return['amount'] == 0) hide @endif payment_row" id="change_return_payment_data">
			...
			@endif
			@include('service::sale_pos.partials.payment_type_details', ['payment_line' => $change_return, 'row_index' => 'change_return'])
		</div>
		@endcomponent
	@endcan
	@endif
	...
	@if(in_array('subscription', $enabled_modules))
		@include('service::sale_pos.partials.recurring_invoice_modal')
	@endif
	{!! Form::close() !!}
</section>

<div class="modal fade contact_modal" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">
	@include('service::contact.create', ['quick_add' => true])
</div>
<!-- /.content -->
...
@include('service::sale_pos.partials.configure_search_modal')
...
@section('javascript')
    ...
    @if(in_array('tables' ,$enabled_modules) || in_array('modifiers' ,$enabled_modules) || in_array('service_staff' ,$enabled_modules))
    	<script src="{{ asset('modules/service/v7/js/restaurant.js?v=' . $asset_v . '-service-jsfix-20260609') }}"></script>
    @endif

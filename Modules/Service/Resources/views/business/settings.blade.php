...
<section class="content-header">
    ...
    <br>
    @include('service::layouts.partials.search_settings')
</section>

<!-- Main content -->
<section class="content">
{!! Form::open(['url' => route('service.business.postBusinessSettings'), 'method' => 'post', 'id' => 'bussiness_edit_form',
           'files' => true ]) !!}
    <div class="row">
        <div class="col-xs-12">
       <!--  <pos-tab-container> -->
        {{-- <div class="col-xs-12 pos-tab-container"> --}}
        @component('service::components.widget', ['class' =>  'pos-tab-container'])
            ...
            <div class="col-lg-10 col-md-10 col-sm-10 col-xs-10 pos-tab">
                <!-- tab 1 start -->
                @include('service::business.partials.settings_business')
                <!-- tab 1 end -->
                <!-- tab 2 start -->
                @include('service::business.partials.settings_tax')
                <!-- tab 2 end -->
                <!-- tab 3 start -->
                @include('service::business.partials.settings_product')

                @include('service::business.partials.settings_contact')
                <!-- tab 3 end -->
                <!-- tab 4 start -->
                @include('service::business.partials.settings_sales')
                @include('service::business.partials.settings_pos')
                @include('service::business.partials.settings_display_pos')
                <!-- tab 4 end -->
                <!-- tab 5 start -->
                @include('service::business.partials.settings_purchase')

                @include('service::business.partials.settings_payment')
                <!-- tab 5 end -->
                <!-- tab 6 start -->
                @include('service::business.partials.settings_dashboard')
                <!-- tab 6 end -->
                <!-- tab 7 start -->
                @include('service::business.partials.settings_system')
                <!-- tab 7 end -->
                <!-- tab 8 start -->
                @include('service::business.partials.settings_prefixes')
                <!-- tab 8 end -->
                <!-- tab 9 start -->
                @include('service::business.partials.settings_email')
                <!-- tab 9 end -->
                <!-- tab 10 start -->
                @include('service::business.partials.settings_sms')
                <!-- tab 10 end -->
                <!-- tab 11 start -->
                @include('service::business.partials.settings_reward_point')
                <!-- tab 11 end -->
                <!-- tab 12 start -->
                @include('service::business.partials.settings_modules')
                <!-- tab 12 end -->
                @include('service::business.partials.settings_custom_labels')
            </div>
        @endcomponent
        {{-- </div> --}}
        <!--  </pos-tab-container> -->
    ...
    $(document).ready(function(){
        ...
        $('#test_email_btn').click( function() {
            ...
            $.ajax({
                method: 'post',
                data: data,
                url: "{{ url(config('service.route_prefix', 'service-pos') . '/test-email') }}",
                dataType: 'json',
        ...
        $('#test_sms_btn').click( function() {
            ...
            $.ajax({
                method: 'post',
                data: data,
                url: "{{ url(config('service.route_prefix', 'service-pos') . '/test-sms') }}",
                dataType: 'json',
                success: function(result) {
                    if (result.success == true) {
                        swal({

...
<section class="content">
	...
	{!! Form::open([
		'url' => action([\Modules\Service\Http\Controllers\DashboardConfiguratorController::class, 'update'], 
		...
		<div class="col-md-3 bg-gray @if(empty($dashboard->configuration)) hide @endif)" id="row_options_div">
			@foreach($available_widgets as $key => $value)
				@include('service::dashboard_configurator.partials.widget', ['widget' => $key])
			@endforeach
		</div>
		...
		<div class="col-md-8 col-md-offset-1">
			...
			<div id="droppable_container">
				@foreach($dashboard->configuration as $row)
					@include('service::dashboard_configurator.partials.row', ['ratio' => $row])
				@endforeach
			</div>

			<div class="mt-10">
				@include('service::dashboard_configurator.partials.row_options')
				<div class="row">

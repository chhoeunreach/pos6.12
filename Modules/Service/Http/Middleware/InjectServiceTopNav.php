...
class InjectServiceTopNav
{
    public function handle(Request $request, Closure $next)
    {
        ...
        if (strpos($content, 'data-service-top-nav="true"') !== false) {
            return $response;
        }
    ...
    protected function shouldInject(Request $request, $response): bool
    {
        ...
        $user = auth()->user();
        if (! $user->can('service.access') && ! $user->can('superadmin') && ! $user->can('product.view')) {
            return false;
        }
    ...
    {
        $url = route('service.home');

        return <<<HTML
                        <a href="{$url}" data-service-top-nav="true"

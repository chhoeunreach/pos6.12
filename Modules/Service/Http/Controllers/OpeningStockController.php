...
class OpeningStockController extends Controller
{
    ...
    public function add($product_id)
    {
        ...
        if (! empty($product) && $product->enable_stock == 1) {
            ...
            if (request()->ajax()) {
                return view('service::opening_stock.ajax_add')
                    ->with(compact(
                        'product',
                        'locations',
                        'purchases',
            ...
            return view('service::opening_stock.add')
                    ->with(compact(
                        'product',
                        'locations',
                        'purchases',
    ...
    public function save(Request $request)
    {
        ...
        return redirect()->action([\Modules\Service\Http\Controllers\ProductController::class, 'index'])->with('status', $output);
    }
}

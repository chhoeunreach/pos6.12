...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (! auth()->user()->can('dashboard.data')) {
            return view('service::home.index');
        }
        ...
        $module_widgets = $this->getServiceModuleData($this->moduleUtil, 'dashboard_widget');
        ...
        return view('service::home.index', compact('sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings', 'is_admin'));
    }
    ...
    public function getPurchasePaymentDues()
    {
        if (request()->ajax()) {
            ...
            return Datatables::of($dues)
                ...
                ->addColumn('action', '@can("purchase.create") <a href="{{action([\Modules\Service\Http\Controllers\TransactionPaymentController::class, \'addPayment\'], [$id])}}" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-accent add_payment_modal"><i class="fas fa-money-bill-alt"></i> @lang("purchase.add_payment")</a> @endcan')
                ->removeColumn('supplier_business_name')
                ->editColumn('supplier', '@if(!empty($supplier_business_name)) {{$supplier_business_name}}, <br> @endif {{$supplier}}')
                ->editColumn('ref_no', function ($row) {
                    if (auth()->user()->can('purchase.view')) {
                        return  '<a href="#" data-href="'.action([\Modules\Service\Http\Controllers\PurchaseController::class, 'show'], [$row->id]).'"
            ...
            return Datatables::of($dues)
                ...
                ->editColumn('invoice_no', function ($row) {
                    if (auth()->user()->can('sell.view')) {
                        return  '<a href="#" data-href="'.action([\Modules\Service\Http\Controllers\SellController::class, 'show'], [$row->id]).'"
                                    class="btn-modal" data-container=".view_modal">'.$row->invoice_no.'</a>';
                    }

                    return $row->invoice_no;
                })
                ->addColumn('action', '@if(auth()->user()->can("sell.create") || auth()->user()->can("direct_sell.access")) <a href="{{action([\Modules\Service\Http\Controllers\TransactionPaymentController::class, \'addPayment\'], [$id])}}" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-accent add_payment_modal"><i class="fas fa-money-bill-alt"></i> @lang("purchase.add_payment")</a> @endif')
    ...
    public function getTotalUnreadNotifications()
    {
        ...
        if (! empty($modal_notifications)) {
            $notification_html = view('service::home.notification_modal')->with(['notifications' => $modal_notifications])->render();
        }
    ...
    public function getCalendar()
    {
        ...
        if (request()->ajax()) {
            ...
            $module_events = $this->getServiceModuleData($this->moduleUtil, 'calendarEvents', $data);
        ...
        $module_event_types = $this->getServiceModuleData($this->moduleUtil, 'eventTypes');
        foreach ($module_event_types as $module_event_type) {
            $event_types = array_merge($event_types, $module_event_type);
        }

        return view('service::home.calendar')->with(compact('all_locations', 'users', 'event_types'));
    }
    ...
    public function showNotification($id)
    {
        ...
        return view('service::home.notification_modal')->with([

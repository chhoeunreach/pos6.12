<!-- app css -->
@if(!empty($for_pdf))
	<link rel="stylesheet" href="{{ asset('modules/accessory/v7/css/app.css?v='.$asset_v) }}">
@endif
		...
		<tbody>
			@foreach($ledger_details['ledger'] as $data)
				...
				@endif>
					...
					<td>
						...
						@if(!empty($is_admin) && !empty($data['transaction_id']) && $data['transaction_type'] == 'ledger_discount')
							<br>
							<button type="button" class="tw-dw-btn tw-dw-btn-outline tw-dw-btn-xs tw-dw-btn-error delete_ledger_discount" data-href="{{action([\Modules\Accessory\Http\Controllers\LedgerDiscountController::class, 'destroy'], ['ledger_discount' => $data['transaction_id']])}}"><i class="fas fa-trash"></i></button>
							<button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-primary btn-modal" data-href="{{action([\Modules\Accessory\Http\Controllers\LedgerDiscountController::class, 'edit'], ['ledger_discount' => $data['transaction_id']])}}" data-container="#edit_ledger_discount_modal"><i class="fas fa-edit"></i></button>
						@endif
					</td>
				</tr>
				@if(!empty($data['transaction_type']) && $data['transaction_type'] == 'sell')
					<tr>
						<td colspan="10" class="bg-light-gray" style="padding: 0 20px 10px;">
							@include('accessory::sale_pos.partials.sale_line_details', ['sell' => (object)$data, 'enabled_modules' => [], 'is_warranty_enabled' => false, 'for_ledger' => true])
						</td>
					</tr>
				@endif
				...
				@if(!empty($data['transaction_type']) && $data['transaction_type'] == 'purchase')
					<tr>
						<td colspan="10" class="bg-light-gray" style="padding: 0 20px 10px;">
							@include('accessory::contact.partials.ledger_purchase_lines_details', ['purchase' => (object)$data])
						</td>
					</tr>
				@endif
			@endforeach

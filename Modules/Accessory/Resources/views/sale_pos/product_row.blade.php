	...
	<td>
		...
		<div style="display:flex;align-items:flex-start;gap:6px;">
			...
			<div style="min-width:0;flex:1;">
				...
				@endif
				<div class="pos-product-meta tw-text-[11px] tw-text-slate-400 tw-leading-[1.3] tw-truncate">{{ $product_sku_brand }}@if($product->enable_stock) &nbsp;·&nbsp; {{ @num_format($product->qty_available) }} {{$product->unit}}@endif</div>
			</div>
		</div>
		...
		<div class="modal fade row_edit_product_price_model" id="row_edit_product_price_modal_{{$row_count}}" tabindex="-1" role="dialog">
			@include('accessory::sale_pos.partials.row_edit_product_price_modal')
		</div> 
		@endif
		...
		@if(in_array('modifiers' , $enabled_modules))
			<div class="modifiers_html">
				@if(!empty($product->product_ms))
					@include('accessory::restaurant.product_modifier_set.modifier_for_product', array('edit_modifiers' => true, 'row_count' => $loop->index, 'product_ms' => $product->product_ms ) )
				@endif
			</div>
		@endif

				...
				<div class="box-header with-border">
					<h3 class="box-title">
						...
						@elseif($transaction->status == 'final') 
							Invoice 
						@endif 
					<span class="text-success">#{{$transaction->invoice_no}}</span> <i class="fa fa-keyboard-o hover-q text-muted" aria-hidden="true" data-container="body" data-toggle="popover" data-placement="bottom" data-content="@include('accessory::sale_pos.partials.keyboard_shortcuts_details')" data-html="true" data-trigger="hover" data-original-title="" title=""></i></h3>
					<p><strong>@lang('sale.location'):</strong> {{$transaction->location->name}}</p>
					<div class="pull-right box-tools">
                <a class="btn btn-success btn-sm" href="{{action([\Modules\Accessory\Http\Controllers\SellPosController::class, 'create'])}}">
                  <strong><i class="fa fa-plus"></i> POS</strong></a>
              </div>
				</div>
				<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
				{!! Form::open(['url' => route('accessory.pos.update', [$transaction->id]), 'method' => 'post', 'id' => 'edit_pos_sell_form' ]) !!}
...
@foreach($sell_details as $sell_line)
	...
	@include('accessory::sale_pos.product_row', 
		['product' => $sell_line, 
...
@endforeach
					...
					@include('accessory::sale_pos.partials.pos_details', ['edit' => true])

					@include('accessory::sale_pos.partials.payment_modal')

					@if(empty($pos_settings['disable_suspend']))
						@include('accessory::sale_pos.partials.suspend_note_modal')
					@endif
		...
		<div class="col-md-5 col-sm-12">
			@include('accessory::sale_pos.partials.right_div')
		</div>
	</div>
</section>
...
@section('javascript')
	...
	@include('accessory::sale_pos.partials.keyboard_shortcuts')

...
<div class="tw-flex tw-items-start tw-gap-2 tw-flex-wrap" style="margin: 0 0 6px 0; padding: 0 4px;">
    ...
    <div class="tw-flex-1 tw-min-w-[140px]" id="sell_list_staff_div">
        <button type="button" id="show_sell_list_staff"
            ...
            <span class="tw-dw-badge tw-dw-badge-sm tw-bg-sky-50 tw-border-sky-100 tw-text-sky-700 tw-font-bold tw-text-[11px] group-hover:tw-bg-white group-hover:tw-border-sky-200 tw-transition-colors sell-list-sidebar-badge">{{ number_format((int) ($hr_sell_out_reports_total ?? 0)) }}</span>
        </button>
    </div>
</div>
    ...
    <div class="col-md-12" style="padding: 0;">
        <div id="product_list_body" class="eq-height-row tw-max-h-[calc(100vh_-_229px)] tw-overflow-y-auto tw-overflow-x-hidden" style="padding-right: 4px;">
            <div id="sell_list_staff_box" style="display: none;">
                @include('accessory::sale_pos.partials.hr_sell_list_staff', ['hr_sell_out_reports' => $hr_sell_out_reports ?? collect(), 'sell_types' => $sell_types ?? collect(), 'default_date_from' => $default_date_from ?? '', 'default_date_to' => $default_date_to ?? ''])
            </div>
            <div id="featured_products_box" style="display: none;">
                @if (!empty($featured_products))
                    @include('accessory::sale_pos.partials.featured_products')
                @endif
            </div>

		...
		@foreach ($transactions as $transaction)
			<tr class="cursor-pointer" 
				...
				<td class="col-md-6 tw-flex tw-flex-col md:tw-flex-row tw-space-x-0 md:tw-space-x-2 tw-space-y-1 md:tw-space-y-0">
					...
					<a href="{{action([\Modules\Accessory\Http\Controllers\SellPosController::class, 'edit'], [$transaction->id]).$subtype}}" class="tw-dw-btn tw-dw-btn-outline tw-dw-btn-info">
					...
					@if(!auth()->user()->can('sell.update') && auth()->user()->can('edit_pos_payment'))
						<a href="{{route('accessory.edit-pos-payment', ['id' => $transaction->id])}}" 
	    			...
	    			<a href="{{action([\Modules\Accessory\Http\Controllers\SellPosController::class, 'printInvoice'], [$transaction->id])}}" class="print-invoice-link tw-dw-btn tw-dw-btn-outline tw-dw-btn-success">
                    ...
                    @if(auth()->user()->can('sell.delete') || auth()->user()->can('direct_sell.delete'))
	    			    <a href="{{action([\Modules\Accessory\Http\Controllers\SellPosController::class, 'destroy'], [$transaction->id])}}" class="delete-sale tw-dw-btn tw-dw-btn-outline tw-dw-btn-error">

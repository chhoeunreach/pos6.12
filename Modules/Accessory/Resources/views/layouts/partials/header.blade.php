    ...
    <div class="tw-px-5 tw-py-3">
        <div class="tw-flex tw-items-start tw-justify-between tw-gap-6 lg:tw-items-center">
            ...
            @if(!empty(session('previous_user_id')) && !empty(session('previous_username')))
                <a href="{{route('accessory.sign-in-as-user', session('previous_user_id'))}}" class="btn btn-flat btn-danger m-8 btn-sm mt-10"><i class="fas fa-undo"></i> @lang('lang_v1.back_to_username', ['username' => session('previous_username')] )</a>
            @endif


            <div class="tw-flex tw-flex-wrap tw-items-center tw-justify-end tw-gap-3">
                @include('accessory::layouts.partials.header-action-button', [
                    'href' => $main_products_url,
                    'icon' => 'arrow-left',
                    'label' => 'Back to Main',
                    'attributes' => [
                        'data-accessory-skip-rewrite' => 'true',
                    ],
                ])
                ...
                <details class="tw-dw-dropdown tw-relative tw-inline-block tw-text-left">
                    ...
                    <ul class="tw-dw-menu tw-dw-dropdown-content tw-dw-z-[1] tw-dw-bg-base-100 tw-dw-rounded-box tw-w-48 tw-absolute tw-left-0 tw-z-10 tw-mt-2 tw-origin-top-right tw-bg-white tw-rounded-lg tw-shadow-lg tw-ring-1 tw-ring-gray-200 focus:tw-outline-none"
                        ...
                        <div class="tw-p-2" role="none">
                            <a href="{{ route('accessory.calendar') }}"
                ...
                <button id="btnCalculator" title="@lang('lang_v1.calculator')" data-content='@include('accessory::layouts.partials.calculator')'
                ...
                @if (in_array('pos_sale', $enabled_modules))
                    @can('sell.create')
                        @include('accessory::layouts.partials.header-action-button', [
                            'href' => '/' . trim(config('accessory.route_prefix', 'accessory-pos'), '/') . '/pos/create',
                            'icon' => 'pos',
                            'label' => __('sale.pos_sale'),
                        ])
                    @endcan
                @endif
                @if (Module::has('SmartStockInventory') && !empty(\App\System::getProperty('smartstockinventory_version')) && Route::has('accessory.ssi.dashboard'))
                    @can('stock_inventory.view')
                        @include('accessory::layouts.partials.header-action-button', [
                            'href' => route('accessory.ssi.dashboard'),
                            'icon' => 'stock-inventory',
                            'label' => 'Stock Inventory',
                        ])
                    @endcan
                @endif
                @if (!in_array('Repair', config('accessory.excluded_main_modules', []), true) && Module::has('Repair'))
                ...
                @include('accessory::layouts.partials.header-notifications')
                ...
                <details class="tw-dw-dropdown tw-relative tw-inline-block tw-text-left">
                    ...
                    <ul class="tw-p-2 tw-w-48 tw-absolute tw-right-0 tw-z-10 tw-mt-2 tw-origin-top-right tw-bg-white tw-rounded-lg tw-shadow-lg tw-ring-1 tw-ring-gray-200 focus:tw-outline-none"
                        ...
                        <li>
                            <a href="{{ action([\Modules\Accessory\Http\Controllers\UserController::class, 'getProfile']) }}"
                                class="tw-flex tw-items-center tw-gap-2 tw-px-3 tw-py-2 tw-text-sm tw-font-medium tw-text-gray-600 tw-transition-all tw-duration-200 tw-rounded-lg hover:tw-text-gray-900 hover:tw-bg-gray-100"
                                role="menuitem" tabindex="-1">
                                <svg aria-hidden="true" class="tw-w-5 tw-h-5" xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" fill="none"
                        ...
                        <li>
                            <a href="{{ action([\Modules\Accessory\Http\Controllers\Auth\LoginController::class, 'logout']) }}"
                                class="tw-flex tw-items-center tw-gap-2 tw-px-3 tw-py-2 tw-text-sm tw-font-medium tw-text-gray-600 tw-transition-all tw-duration-200 tw-rounded-lg hover:tw-text-gray-900 hover:tw-bg-gray-100"
                                role="menuitem" tabindex="-1">
                                <svg aria-hidden="true" class="tw-w-5 tw-h-5" xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" fill="none"

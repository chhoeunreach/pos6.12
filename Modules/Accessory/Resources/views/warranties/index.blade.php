...
@section('content')
    ...
    <section class="content">
        @component('accessory::components.widget', ['class' => 'box-primary', 'title' => __('lang_v1.all_warranties')])
            @slot('tool')
                <div class="box-tools">
                    <a class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full btn-modal pull-right"
                        data-href="{{action([\Modules\Accessory\Http\Controllers\WarrantyController::class, 'create'])}}" 
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var warranty_table = $('#warranty_table').DataTable({
                ...
                serverSide: true,
                fixedHeader:false,
                ajax: "{{ action([\Modules\Accessory\Http\Controllers\WarrantyController::class, 'index']) }}",
                columnDefs: [{
                    "targets": 3,
                    "orderable": false,
                    "searchable": false

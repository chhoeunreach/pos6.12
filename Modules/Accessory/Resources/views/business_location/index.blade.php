...
<section class="content">
    @component('accessory::components.widget', ['class' => 'box-primary', 'title' => __( 'business.all_your_business_locations' )])
        @slot('tool')
            <div class="box-tools">
                <a class="tw-dw-btn tw-dw-btn-sm tw-dw-btn-outline tw-dw-btn-primary pull-right tw-mb-2 tw-ml-2"
                    href="{{ action([\Modules\Accessory\Http\Controllers\BusinessLocationController::class, 'downloadTemplate']) }}">
                ...
                <button class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right tw-mb-2 btn-modal"
                    data-href="{{action([\Modules\Accessory\Http\Controllers\BusinessLocationController::class, 'create'])}}" 
...
<script>
    $(document).ready(function() {
        ...
        $('#bl_preview_btn').click(function() {
            ...
            $.ajax({
                method: 'POST',
                url: '{{ action([\Modules\Accessory\Http\Controllers\BusinessLocationController::class, 'importPreview']) }}',
                data: formData,
                processData: false,
                contentType: false,
                dataType: 'json',
        ...
        $('#bl_confirm_import_btn').click(function() {
            ...
            $.ajax({
                method: 'POST',
                url: '{{ action([\Modules\Accessory\Http\Controllers\BusinessLocationController::class, 'importConfirm']) }}',
                dataType: 'json',
                data: {
                    _token: '{{ csrf_token() }}',
                    token: token,
        ...
        $('#bl_export_btn').click(function() {
            ...
            var url = '{{ action([\Modules\Accessory\Http\Controllers\BusinessLocationController::class, 'export']) }}'

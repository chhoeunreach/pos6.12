...
@section('content')
    ...
    <section class="content">
                            ...
                            <div class="tab-pane active " id="product_list_tab">
                                @if ($is_admin)
                                    ...
                                    <a class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right tw-m-2"
                                        href="{{ action([\Modules\Accessory\Http\Controllers\ProductController::class, 'downloadExcel']) }}">
                                ...
                                @can('product.create')
                                    ...
                                    <a class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right tw-m-2"
                                        href="{{ action([\Modules\Accessory\Http\Controllers\ProductController::class, 'create']) }}">
                                    ...
                                    <br><br>
                                @endcan
                                @include('accessory::product.partials.product_list')
                            </div>
                            @can('stock_report.view')
                                <div class="tab-pane" id="product_stock_report">
                                    @include('accessory::report.partials.stock_report_table')
                                </div>
                            @endcan
                        </div>
                    </div>
        ...
        @if ($is_woocommerce)
            @include('accessory::product.partials.toggle_woocommerce_sync_modal')
        @endif
        @include('accessory::product.partials.edit_product_location_modal')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            product_table = $('#product_table').DataTable({
                ...
                scrollCollapse: true,
                "ajax": {
                    "url": "{{ action([\Modules\Accessory\Http\Controllers\ProductController::class, 'index']) }}",
                    "data": function(d) {
        ...
        $(document).on('shown.bs.modal', 'div.view_product_modal, div.view_modal, #view_product_modal',
            function() {
                ...
                if (div.length) {
                    $.ajax({
                        url: "{{ action([\Modules\Accessory\Http\Controllers\ReportController::class, 'getStockReport']) }}" +

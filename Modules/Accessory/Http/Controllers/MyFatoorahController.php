...
//-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    private function getPayLoadData($package_id = null, $amount, $currency, $coupon_code, $email, $name, $user_id, $business_id, $language) {
        $callbackURL = route('accessory.myfatoorah_callback');
...
//-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    public function checkout() {
        try {
            ...
            return view('accessory::myfatoorah.checkout', compact('mfSession', 'paymentMethods', 'jsDomain', 'userDefinedField'));
        } catch (Exception $ex) {
            $exMessage = __('myfatoorah.' . $ex->getMessage());
            return view('accessory::myfatoorah.error', compact('exMessage'));
        }
    }

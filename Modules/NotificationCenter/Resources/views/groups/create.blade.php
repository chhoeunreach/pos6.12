...
<div class="nc-page">
    <div class="nc-shell">
        <div class="nc-heading">
            <div>
                <h1 class="nc-title">Add Telegram Group</h1>
                <p class="nc-subtitle">Create a channel route for Notification Center messages.</p>
            </div>
        ...
        <form action="{{ route('notificationcenter.groups.store') }}" method="POST" class="nc-form">
            @csrf
            <div class="nc-form-body">
                <div class="nc-grid">
                    <div class="nc-field-full">
                        <label class="nc-label"><i class="fa fa-users"></i> Group Name</label>
                        <input type="text" name="name" value="{{ old('name') }}" required class="nc-control" placeholder="e.g. Main Stock Channel">
                        @error('name')<p class="nc-error">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label class="nc-label"><i class="fa fa-paper-plane"></i> Chat ID</label>
                        <input type="text" name="chat_id" value="{{ old('chat_id') }}" required class="nc-control" placeholder="e.g. -1001234567890">
                        <p class="nc-help">Negative for groups/channels, positive for users.</p>
                    ...
                    <div>
                        ...
                        <select name="location_id" class="nc-control select2">
                            @foreach($business_locations as $id => $name)
                                <option value="{{ $id }}" @if((string) old('location_id') === (string) $id) selected @endif>{{ $name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="nc-options">
                    <label class="nc-option">
                        <input type="checkbox" name="send_text" value="1" checked>
                        <span><i class="fa fa-commenting-o"></i> Send Text</span>
                    </label>
                    <label class="nc-option">
                        <input type="checkbox" name="send_pdf" value="1" checked>

...
<div class="tw-px-3 lg:tw-px-5 tw-mx-auto tw-max-w-3xl">
    ...
    <form action="{{ route('notificationcenter.templates.update', $template->id) }}" method="POST" class="tw-bg-white tw-rounded-lg tw-shadow-sm tw-border tw-p-6 tw-space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ $template->title }}" required class="tw-w-full tw-border tw-border-gray-300 tw-rounded-lg tw-px-3 tw-py-2 tw-text-sm">
        </div>
        <div>
            ...
            <select name="module_type" required class="tw-w-full tw-border tw-border-gray-300 tw-rounded-lg tw-px-3 tw-py-2 tw-text-sm">
                <option value="stock_transfer" @if($template->module_type === 'stock_transfer') selected @endif>Stock Transfer</option>
                <option value="loan_payment" @if($template->module_type === 'loan_payment') selected @endif>Loan Payment</option>
                <option value="loan_installment" @if($template->module_type === 'loan_installment') selected @endif>Loan Installment</option>
            </select>
        </div>
        <div>
            <label class="tw-block tw-text-sm tw-font-medium tw-text-gray-700">Message Template</label>
            <textarea name="message_template" rows="6" required class="tw-w-full tw-border tw-border-gray-300 tw-rounded-lg tw-px-3 tw-py-2 tw-text-sm">{{ $template->message_template }}</textarea>
        </div>
        <div>
            <label class="tw-block tw-text-sm tw-font-medium tw-text-gray-700">PDF Template View</label>
            <input type="text" name="pdf_template_view" value="{{ $template->pdf_template_view }}" class="tw-w-full tw-border tw-border-gray-300 tw-rounded-lg tw-px-3 tw-py-2 tw-text-sm">
        </div>
        <div>

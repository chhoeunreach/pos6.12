...
<form method="post" action="{{ ssi_route('ssi.settings.update') }}">@csrf
...
<div class="col-md-5" style="margin-top:24px;">
    <button formaction="{{ ssi_route('ssi.settings.test_telegram') }}" formmethod="post" class="btn btn-info">Test Telegram</button>
    <button formaction="{{ ssi_route('ssi.settings.reset_default') }}" formmethod="post" class="btn btn-warning">Reset Default</button>
    <a class="btn btn-success" href="{{ ssi_route('ssi.settings.export') }}">Export Settings</a>
</div>
</div></div>
</form>
@endsection

...
<div class="col-md-3">
    <select class="form-control select2" name="location_id" id="dashboard_location">
        <option value="">{{ __('lang_v1.select_location') }}</option>
        @foreach($locations as $loc)
            <option value="{{ $loc->id }}" {{ in_array($loc->id, $locationIds ?? []) ? 'selected' : '' }}>{{ $loc->name }}</option>
        @endforeach
    </select>
</div>
<div class="col-md-3">
...
$(function(){
    ...
    setDates(start,end);
    $('#dashboard_location').on('change', function(){ $('#ssi_dashboard_filter_form').submit(); });
});
</script>
@endsection

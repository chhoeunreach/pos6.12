...
<form method="post" action="{{ ssi_route('ssi.count.enterprise.session') }}">@csrf
<div class="row">
...
<td>
<a class="btn btn-xs btn-info" href="{{ ssi_route('ssi.count.enterprise.mobile', $s->id) }}">Mobile</a>
<a class="btn btn-xs btn-default" href="{{ ssi_route('ssi.count.enterprise.adjustment_preview', $s->id) }}">Adjustment Preview</a>
<a class="btn btn-xs btn-warning" href="{{ ssi_route('ssi.count.enterprise.dashboard', $s->id) }}" target="_blank">Live Dashboard JSON</a>
<form method="post" action="{{ ssi_route('ssi.count.enterprise.approve', $s->id) }}" style="display:inline-block;">@csrf<input type="hidden" name="approval_level" value="manager"><button class="btn btn-xs btn-success">Approve</button></form>
</td>
</tr>
@endforeach
</tbody></table>

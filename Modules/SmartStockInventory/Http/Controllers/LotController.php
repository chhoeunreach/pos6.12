...
class LotController extends BaseSmartStockController
{
    ...
    public function updateLot(Request $request)
    {
        ...
        $row = DB::table('smart_lot_histories')->where('business_id', $this->businessId())->where('id', $d['id'])->first();
        abort_unless($row, 404);
        DB::table('smart_lot_histories')->where('id', $d['id'])->update(['lot_number' => $d['lot_number']]);
        SmartStockActionLog::create(['user_id' => auth()->id(), 'business_id' => $this->businessId(), 'location_id' => $row->location_id, 'action_type' => 'update_lot_detail', 'reference_type' => 'smart_lot_histories', 'reference_id' => $row->id, 'old_data' => json_encode($row), 'new_data' => json_encode(['lot_number' => $d['lot_number']]), 'reason' => $d['reason']]);
    ...
    public function export(Request $request)
    {
        ...
        $rows = DB::table('smart_lot_histories')->where('business_id', $this->businessId())->orderByDesc('movement_date')->limit(10000)->get()->map(fn ($r) => (array) $r)->all();
    ...
    {
        abort_unless($request->user()->can('stock_inventory.view'), 403);
        $rows = DB::table('smart_lot_histories')->where('business_id', $this->businessId())->where('lot_number', $lot)->orderByDesc('movement_date')->paginate(200);

...
class SmartStockAccessMiddleware
{
    ...
    {
        if (! $this->isInstalledForCurrentContext($request)) {
            abort(404);
        }
        ...
        $routeName = $this->baseRouteName((string) optional($request->route())->getName());
    ...
    private function isInstalledForCurrentContext(Request $request): bool
    {
        ...
        if (! $request->is($accessoryPrefix) && ! $request->is($accessoryPrefix.'/*')) {
            return false;
        }

        $statusPath = module_path('Accessory', 'modules_statuses.json');
        if (! is_file($statusPath)) {
            return false;
        }

        $statuses = json_decode((string) file_get_contents($statusPath), true);
        return ! empty($statuses['SmartStockInventory']);
    }

    private function baseRouteName(string $routeName): string
    {
        foreach (['accessory.', 'service.'] as $prefix) {
    ...
    private function requiredPermissionForRoute(string $routeName): ?string
    {
        $map = [
            ...
            'ssi.report.stock_sell' => 'stock_report.view',
            'ssi.report.stock_purchase' => 'stock_report.view',
            'ssi.report.stock_transfer' => 'stock_report.view',

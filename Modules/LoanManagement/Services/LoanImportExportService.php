...
ប្រចាំខែ' => 'monthly_interest',
    ...
    protected function templateDefinitions(): array
    {
        return [
            'customers' => [
                'columns' => ['customer_code', 'customer_name', 'customer_group', 'phone', 'alternate_phone', 'email', 'national_id', 'address', 'occupation', 'employer_name', 'employer_phone', 'status', 'note'],
                'required' => ['customer_name or phone'],
                'optional' => ['customer_code', 'customer_group', 'alternate_phone', 'email', 'national_id', 'address', 'occupation', 'employer_name', 'employer_phone', 'status', 'note'],
                'example' => ['CUS-0001', 'Sok Dara', 'VIP', '012345678', '011222333', 'sok@example.com', '010101234', 'Phnom Penh', 'Seller', 'ABC Shop', '023555111', 'active', 'Imported customer'],
            ...
            'schedules' => [
                ...
                'optional' => ['status', 'paid_date'],
                'example' => ['LN-0001', '', '1', now()->addMonth()->toDateString(), '50.00', '5.00', '55.00', 'unpaid', ''],
                'notes' => 'Updates an existing loan schedule with same loan and installment_no when found, otherwise creates one.',
            ],
            'payments' => [
                'columns' => ['loan_invoice', 'payment_date', 'amount', 'cash_amount', 'bank_amount', 'payoff_amount', 'payment_method', 'reference_no', 'received_by', 'note'],
                'required' => ['loan_invoice', 'payment_date', 'amount'],
                'optional' => ['cash_amount', 'bank_amount', 'payoff_amount', 'payment_method', 'reference_no', 'received_by', 'note'],
                'example' => ['KY-000001', now()->toDateString(), '55.00', '0.00', '50.00', '5.00', 'Bank', 'PAY-KY-000001-'.now()->format('Ymd').'-M1-CASH', 'Admin', 'Monthly installment payment'],
                'notes' => 'If schedule_id is empty, payment is applied to the oldest unpaid schedule for the loan. Use amount, or split the same total across cash_amount, bank_amount, and payoff_amount. Duplicate payments match by reference_number when provided, otherwise by loan + schedule + payment_type + paid_date + amount + payment_method.',
            ],
            'guarantors' => [
    ...
    protected function storePayment(array $row, string $duplicateMode = 'skip', ?int $existingPaymentId = null): int
    {
        ...
        $payload = $this->safeColumns('loan_payments', [
            ...
            'received_by_name_snapshot' => $row['received_by'],
            'collected_by_name_snapshot' => $row['received_by'],
    ...
    protected function refreshLoanTotals(int $loanId): void
    {
        ...
        $principalPaid = $this->hasColumn('loan_payments', 'principal_paid')
            ? (float) DB::connection($this->connection)->table('loan_payments')->where('loan_id', $loanId)->sum('principal_paid')
            : min($paid, (float) ($loan->principal_amount ?? 0));
        $interestPaid = $this->hasColumn('loan_payments', 'interest_paid')
            ? (float) DB::connection($this->connection)->table('loan_payments')->where('loan_id', $loanId)->sum('interest_paid')
    ...
    protected function exportColumns(string $type): array
    {
        $loanColumns = [
            ...
            'created_by', 'approved_by', 'loan_status', 'created_at',
        ...
        $columns = [
            ...
            'payments' => ['loan_invoice', 'payment_date', 'amount', 'cash_amount', 'bank_amount', 'payoff_amount', 'payment_method', 'reference_no', 'received_by', 'note'],
            'monthly_collections' => ['loan_invoice', 'payment_date', 'amount', 'cash_amount', 'bank_amount', 'payoff_amount', 'payment_method', 'reference_no', 'received_by', 'note'],

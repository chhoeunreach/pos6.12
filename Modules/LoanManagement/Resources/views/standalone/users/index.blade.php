...
<div class="lm-users-content">
    ...
    <section class="content-header" style="padding: 0 0 16px 0; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
        ...
        <div style="display: flex; gap: 8px; flex-wrap: wrap;">
            @can('roles.view')
                <a href="{{ route('roles.index') }}" class="btn btn-default btn-sm" style="font-weight: 600; border-radius: 6px;">
                    <i class="fa fa-shield"></i> {{ $lmText('Roles', 'តួនាទី') }}
                </a>
            @endcan
            @can('user.create')
                <a href="{{ route('users.import-template') }}" class="btn btn-default btn-sm" style="font-weight: 600; border-radius: 6px;">
            ...
            @endcan
            @can('user.view')
                <a href="{{ route('users.export') }}" class="btn btn-default btn-sm" style="font-weight: 600; border-radius: 6px;">
                    <i class="fa fa-file-excel-o"></i> {{ $lmText('Export', 'នាំចេញ') }}
                </a>
            @endcan
            @can('user.create')
                <a href="{{ route('users.create') }}" class="btn btn-primary btn-sm" style="font-weight: 700; border-radius: 6px;">
        ...
        <form method="GET" action="{{ route('users.index') }}" id="usersFilterForm">
            <div class="lm-pos-filter-grid">
                ...
                <div class="lm-pos-filter-actions">
                    ...
                    <a href="{{ route('users.index') }}" class="lm-btn-pos-reset">
                        ...
                        <tr>
                            ...
                            <td style="text-align: center; vertical-align: middle;">
                                <div class="btn-group" style="display: inline-flex; gap: 3px;">
                                    @can('user.update')
                                        <a href="{{ route('users.edit', $user->id) }}" class="btn btn-xs btn-primary" title="{{ $lmText('Edit User', 'កែប្រែ') }}">
                                    ...
                                    @can('user.delete')
                                        @if(auth()->id() !== $user->id)
                                            <form method="POST" action="{{ route('users.destroy', $user->id) }}" style="display: inline;" onsubmit="return confirm('{{ $lmText('Are you sure you want to delete this user?', 'តើអ្នកប្រាកដជាចង់លុបអ្នកប្រើនេះ?') }}');">
                                                @csrf
                                                @method('DELETE')
...
@can('user.create')
    <div class="modal fade" id="usersImportModal" tabindex="-1" role="dialog" aria-labelledby="usersImportModalLabel">
        <div class="modal-dialog" role="document">
            <form method="POST" action="{{ route('users.import') }}" enctype="multipart/form-data" class="modal-content" style="border-radius: 10px; overflow: hidden;">
                @csrf

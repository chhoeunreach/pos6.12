...
<div class="pos-admin-page">
    <div class="pos-page-head">
        ...
        <div class="pos-action-row">
            @can('user.view')
                <a href="{{ route('loan-management.users.index') }}" class="btn btn-default btn-sm"><i class="fa fa-users"></i> Users</a>
            @endcan
            @can('roles.create')
                <a href="{{ route('loan-management.roles.import-template') }}" class="btn btn-default btn-sm"><i class="fa fa-download"></i> Template</a>
            ...
            @endcan
            @can('roles.view')
                <a href="{{ route('loan-management.roles.export') }}" class="btn btn-default btn-sm"><i class="fa fa-file-excel-o"></i> Export</a>
            @endcan
            @can('roles.create')
                <a href="{{ route('loan-management.roles.create') }}" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> Add Role</a>
            @endcan
        </div>
    </div>
                ...
                <tbody>
                    @forelse($roles as $role)
                        <tr>
                            ...
                            <td>
                                @can('roles.update')
                                    <a href="{{ route('loan-management.roles.edit', $role->id) }}" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i> Edit</a>
                                @endcan
                                @can('roles.delete')
                                    <form method="POST" action="{{ route('loan-management.roles.destroy', $role->id) }}" style="display:inline;" onsubmit="return confirm('Delete this role?');">
                                        @csrf
                                        @method('DELETE')
    ...
    @can('roles.create')
        <div class="modal fade" id="rolesImportModal" tabindex="-1" role="dialog" aria-labelledby="rolesImportModalLabel">
            <div class="modal-dialog" role="document">
                <form method="POST" action="{{ route('loan-management.roles.import') }}" enctype="multipart/form-data" class="modal-content">
                    @csrf

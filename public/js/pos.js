...
let global_p_category_id = null;
let global_is_clear_local_storage = false;
...
$(document).ready(function() {
    ...
    $('select#select_location_id').change(function() {
        ...
        if ($hrBranchMap.length) {
            ...
            var entry = map[newLocId] || {};
            var hrBranch = (typeof entry === 'string') ? entry : (entry.branch || '');
            var hrSellType = (typeof entry === 'string') ? '' : (entry.sell_type || '');
            var $branchFilter = $('.sell-list-filter-branch');
            if ($branchFilter.length) {
                $branchFilter.val(hrBranch);
            }
            var $sellTypeFilter = $('.sell-list-filter-sell-type');
            if ($sellTypeFilter.length) {
                $sellTypeFilter.val(hrSellType);
            }
        }
    });
    ...
    $('button#pos-quotation').click(function() {
        ...
        $.ajax({
            ...
            error: function() {
                enable_pos_form_actions();
                toastr.error('Failed to save quotation');
            },
        });
    });
        ...
        submitHandler: function(form) {
            ...
            if (cnf) {
                ...
                $.ajax({
                    ...
                    error: function() {
                        enable_pos_form_actions();
                        toastr.error('Failed to finalize invoice');
                    },
                });
            }
            return false;
...
// on click sub category in category drawer
    ...
    $(document).on('click', 'button.pos_add_quick_product', function() {
        ...
        $.ajax({
            ...
            error: function() {
                toastr.error('Failed to load quick add product form');
            },
        });
    });
    ...
    $(document).on('click', '.sell-list-copy-all-btn', function(){
        ...
        function process_next(index) {
            ...
            $.getJSON('/sells/pos/resolve-hr-line-product', {
                ...
                stopped = true;
                $btn.prop('disabled', false).text('Copy');
                toastr.error('Failed to resolve HR line product');
                get_hr_sell_list();
            });
        }
...
function process_hr_lines_sequential(lines, index, onComplete) {
    if (index >= lines.length) {
    ...
    var line = lines[index];
    if (!line.serial || is_hr_serial_already_in_current_invoice(line.serial)) {
    ...
    var search_fields = ['name', 'sku', 'sub_sku'];
    ...
    $.getJSON('/products/list', {
        ...
        if (data.auto_add && data.row_data && data.row_data.success) {
            ...
            toastr.error(line.serial + ': product not found in POS.');
        }
...
function release_hr_sell_list_line_from_row($row) {
    ...
    $.ajax({
        ...
        error: function() {
            console.error('Failed to release HR sell list line');
        },
    });
}
...
function get_recent_transactions(status, element_obj) {
    ...
    $.ajax({
        ...
        error: function() {
            console.error('Failed to get recent transactions');
        },
    });
}
...
function pos_add_product_row_from_data(result, insert_after_row) {
    if (result.success) {
        ...
        if (item_addtn_method != 0 && variation_id) {
            var is_added = false;
            var $pos_table_body = $('#pos_table tbody');
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1, insert_after_row = null) {
    ...
    if (add_via_ajax) {
        ...
        $.ajax({
            ...
            error: function() {
                toastr.error('Failed to get product row');
            },
        });
    }
}
...
function getCustomerRewardPoints() {
    ...
    $.ajax({
        ...
        error: function() {
            console.error('Failed to get reward details');
        },
    });
}
...
$(document).on('change', '#types_of_service_id', function(){
    ...
    if(types_of_service_id) {
        $.ajax({
            ...
            error: function() {
                console.error('Failed to get types of service details');
            },
        });
    } else {
...
function validate_discount_field() {
    var discount_element = $('#discount_amount_modal');
...
function get_sales_orders() {
    if ($('#sales_order_ids').length) {
        ...
        $.ajax({
            ...
            error: function() {
                console.error('Failed to get sales orders');
            },
        });
    }
}
...
$("#sales_order_ids").on("select2:select", function (e) {
    ...
    $.ajax({
        ...
        error: function() {
            toastr.error('Failed to load sales order lines');
        },
    });
});
...
$(document).on('click', '#add_expense', function(){
    $.ajax({
        ...
        error: function() {
            toastr.error('Failed to load expense form');
        },
    });
});
...
$(document).on('submit', 'form#add_expense_modal_form', function(e) {
    ...
    $.ajax({
        ...
        error: function() {
            toastr.error('Failed to save expense');
        },
    });
});
...
function get_contact_due(id) {
    $.ajax({
        ...
        error: function() {
            console.error('Failed to get contact due');
        },
    });
}
...
$(document).on('click', '#send_for_sell_return', function(e) {
    ...
    if (invoice_no) {
        $.ajax({
            ...
            error: function() {
                toastr.error('Failed to validate invoice for return');
            },
        });
    }
})
    ...
    $(document).on('click', '#send_for_sercice_staff_replacement', function (e) {
        ...
        if (invoice_no) {
            $.ajax({
                ...
                error: function() {
                    toastr.error('Failed to validate invoice');
                },
            });
        }
    });
    ...
    $(document).on('submit', 'form#change_service_staff', function (e) {
        ...
        $.ajax({
            ...
            error: function() {
                toastr.error('Failed to change service staff');
            },
        });
    });
...
$(document).on('click', 'button.pause_resume_timer', function(e){
    ...
    $.ajax({
        ...
        error: function() {
            $('.view_modal').find('.overlay').addClass('hide');
        },
    });
})
...
function loadServiceStaffAvailability(show = true) {
    ...
    $.ajax({
        ...
        error: function() {
            console.error('Failed to load service staff availability');
        },
    });
}
